...
def main():
    ...
    if args.datasets_config:
        if Path(args.datasets_config).exists():
            with open(args.datasets_config, "r") as f:
                datasets_config = json.load(f)
        else:
            print(f"Error: Config file not found: {args.datasets_config}")
            sys.exit(1)
    else:
        print("Warning: No datasets config provided. Using default Food-101 only.")
        # Default: Food-101 only
        datasets_config = [
            {
                "name": "food101",
                "path": str(Path(__file__).parent / "data" / "food-101"),
    ...
    # Download Food-101 if it's in the config
    food101_config = next(
        (d for d in datasets_config if d.get("name", "").lower() == "food101"), None
    )
    if food101_config:
        print("Checking Food-101 dataset...")
        try:
            download_food101_data(config.DATA_DIR)
            print()
        except Exception as e:
            print(f"Warning: Could not download Food-101: {e}\n")
