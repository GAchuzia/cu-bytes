...
def log_food_item_json(data):
    ...
    if not isinstance(username, str) or len(username) == 0:
        return (
            jsonify(
                {"status": "error", "message": "username must be a non-empty string"}
            ),
            400,
        )
    ...
    if not isinstance(food_id, int):
        return (
            jsonify({"status": "error", "message": "food_id must be an integer"}),
            400,
        )

    if not isinstance(calories, int):
        return (
            jsonify({"status": "error", "message": "calories must be an integer"}),
            400,
        )
