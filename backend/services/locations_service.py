...
def get_all_dining_locations_json():
    try:
        ...
        result = [
            {"id": location.dining_service_id, "name": location.dining_location_name}
...
def get_dining_location_by_id_json(dining_location_id):
    try:
        ...
        food_items = FoodItem.query.filter(
            FoodItem.dining_location == dining_location_id
        ).all()
        result = [{"id": item.id, "name": item.food_name} for item in food_items]

        if result == []:
            return {
                "error": f"Dining location with id {dining_location_id} not found"
            }, 400
        else:
            return jsonify(result), 200
        ...
        print(
            "LocationService: Error retrieving dining location "
            f"food items information: {e}"
        )
        return (
            jsonify(
                {"error": "Failed to retrieve dining location food items information"}
            ),
            500,
        )
